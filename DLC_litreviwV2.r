...
base2 = ggplot(data = dflong2, aes(x =device,y=participation_adj, shape = group2, colour = group2))+
  geom_point(size = 4)+
  scale_colour_manual(name = "Legend",#Type of DLC participation result",
  ...
  scale_shape_manual(name = "Legend",#"Type of DLC participation result",
                     labels = c("Hypothetical adj.| payment offered", "Hypothetical adj.| no payment", "Not hypothetical | payment offered", "Not hypothetical | no payment","Average participation"),
                     values = c(19, 19,17, 17, 18))+
  ylim(0,.6)+ ggrepel::geom_text_repel(data = dflong2, aes(label = pubID), color = "black", size = 3,show.legend = F)

base2 = base2 + scale_y_continuous(labels = percent)
base2
...
styled2 <- labelled2 +
  ...
  theme(
    plot.title = element_text(face = "bold", size = 12),
    legend.background = element_rect(fill = "white", size = 1, colour = "black"),
    legend.justification = c(0, 1),
